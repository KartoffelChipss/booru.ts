...
describe('Gelbooru (integration)', () => {
    it('requires credentials', () => {
        expect(() => new Gelbooru({ apiKey: '', userId: '' })).toThrow(
            /requires an apiKey and userId/
        );
    });

    it('exposes site metadata', () => {
        const site = new Gelbooru({ apiKey: 'key', userId: '1' });
        expect(site.getName()).toBe('Gelbooru');
        expect(site.getSlug()).toBe('gelbooru');
        expect(site.getWebsite()).toBe('https://gelbooru.com');
    ...
    it('autocompletes tags matching the query', async () => {
        const site = new Gelbooru({ apiKey: 'dummy', userId: '1' });
    ...
    // gelbooru.com requires a real account's api_key + user_id for every
    // API request, so the tests below only run when GELBOORU_API_KEY and
    // GELBOORU_USER_ID are set in the environment.
    ...
    describeWithCredentials('with valid credentials', () => {
        ...
        // the Gelbooru constructor with missing credentials.
        const getSite = () =>
            new Gelbooru({
