...
A TypeScript library for interacting with Booru style image board APIs such as Gelbooru, Rule34, Safebooru, XBooru, HypnoHub, Danbooru, AIBooru, Paheal, yande.re and Konachan. It gives you one consistent interface for searching posts and autocompleting tags, so you do not have to deal with each site's own API quirks yourself.
...
- Gelbooru (gelbooru.com), requires an API key and user id
...
- AIBooru (aibooru.online), login and API key optional, but raise the tag limit
...
Gelbooru requires an API key and user id for every request. Get them from your account settings on gelbooru.com.

```ts
import { Gelbooru } from 'booru.ts';

const site = new Gelbooru({
    apiKey: 'your-api-key',
    userId: 'your-user-id',
});
...
AIBooru works without credentials, but anonymous searches are limited to 4 tags. Passing a login and API key raises that limit.

```ts
import { AIBooru } from 'booru.ts';

const site = new AIBooru();
const authedSite = new AIBooru({
    login: 'your-login',
    apiKey: 'your-api-key',
});
...
### Safebooru, XBooru, HypnoHub, Paheal, yande.re and Konachan

All six work without any credentials.
